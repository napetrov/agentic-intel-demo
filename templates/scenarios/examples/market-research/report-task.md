...
3. submit an offload job through Control Plane (`POST /offload`)
4. poll Control Plane for completion (`GET /offload/{job_id}`)
5. fetch the artifact via Control Plane relay (`GET /artifacts/{ref}`)
6. assemble the analyst-style summary

The offload + artifact-relay endpoints are implemented by the control-plane
offload relay in `runtimes/control-plane/` and exercised end-to-end by the
`tier1-scenario-slice` CI job. Match the abstraction level used by the
live `agents/scenarios/*` files when authoring a real scenario.
